...
export default {
  ...
  methods: {
    async fetchTopics(page = 0) {
      try {
        const response = await axios.get("http://election-backend-cmsa.onrender.com/api/forum/topics", {
          params: { page, size: 5 },
        });
    ...
    async submitReply(topicId) {
      try {
        ...
        const response = await axios.post(`http://election-backend-cmsa.onrender.com/api/forum/topics/${topicId}/replies`, {
